...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        # copying weights from local to target networks
...
def ddpg(n_episodes=2000):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if i_episode % 100 == 0:
            ...
            f1.write('\rEpisode {}\tAverage Score: {:.2f}'.format(i_episode, avg))
        if ((i_episode > 100) and (avg > 30)):
                ...
                f1.write('\rAverage Score of {:.2f} reached. Environment solved in {} episodes.'.format(np.mean(scores_deque), i_episode))
                torch.save(agent.actor_local.state_dict(), 'checkpoint_actor.pth')
                torch.save(agent.critic_local.state_dict(), 'checkpoint_critic.pth')
                break
...
f1 = open("prints.txt", "w")
f1.close() 

f1 = open("prints.txt", "a")

final_scores = ddpg(700)
